...
function _visualize(gene, variant, mainId, input, info) {
  ...
  if ($(`#${id.main}`).length == 0)
    throw "Violin Plot Error: the chart DOM doesn't exist";
  if ($(`#${id.tooltip}`).length == 0)
    $("<div/>").attr("id", id.tooltip).appendTo($("body"));
...
function _customizeViolinPlot(plot, dom) {
  plot.groups.forEach((g) => {
    ...
    const customLabels = customXlabel
...
function _submit(
  ...
  return function () {
    ...
    if (pairs.length == 0) {
      ...
      $(`#${messageBoxId}`).append(
        `Your input has exceeded the maximum number of allowed entries. Only the first ${max} entries are processed.`
      );
      console.warn(
        "User input has exceeded the maximum number of allowed entries."
      );
    ...
    pairs.forEach(function (pair, i) {
      ...
      const variantUrl = vid.toLowerCase().startsWith("rs")
        ? urls.snp + vid
        : urls.variantId + vid;

      Promise.all([
        json(geneUrl, { credentials: "include" }),
        json(variantUrl, { credentials: "include" }),
      ])
        .then(function (args) {
        ...
        .catch(function (err) {
          ...
          if (
            validPairsProcessed === 0 &&
            $(`#${messageBoxId}`).text().length > 0
          ) {
...
function _renderEqtlPlot(
  ...
  // Our IDs don't include ref/alt, so get from API instead.
      ...
      results.forEach((d) => {
        if (d.status == "failed") {
          ...
          input = input.concat([
            {
              group: group,
              label: d.ref.length > 2 ? "ref" : d.ref,
              size: d.homoRefExp.length,
              values: d.homoRefExp,
            },
            {
              group: group,
              label: d.het.length > 2 ? "het" : d.het,
              size: d.heteroExp.length,
              values: d.heteroExp,
            },
            {
              group: group,
              label: d.alt.length > 2 ? "alt" : d.alt,
          ...
          info[group] = {
            pvalue:
              d["pValue"] === null
                ? null
                : parseFloat(d["pValue"]).toPrecision(3),
            pvalueThreshold:
              d["pValueThreshold"] === null
                ? 0
                : parseFloat(d["pValueThreshold"]).toPrecision(3),
          };
        }
      });
