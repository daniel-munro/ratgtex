...
import { getGtexUrls, parseTissues } from "./modules/DataParser";
...
export function launch(
  ...
  tooltipRootId,
  geneId,
  plotTitle = "Gene Expression",
  urls = getGtexUrls(),
  margins = _setViolinPlotMargins(50, 75, 250, 60),
  dimensions = { w: $(`#${rootId}`).innerWidth() * 0.9, h: 250 }
) {
  const promises = [
  ...
  if ($(`#${ids.root}`).length == 0)
    throw "Violin Plot Error: rootId does not exist.";
  // create DOM components if not already present
  if ($(`#${ids.tooltip}`).length == 0)
    $("<div/>").attr("id", ids.tooltip).appendTo($("body"));
  if ($(`#${ids.toolbar}`).length == 0)
    $("<div/>")
      .attr("id", ids.toolbar)
      .appendTo($(`#${ids.root}`));
  if ($(`#${ids.root} #${ids.spinner}`).length == 0)
    $(`<span><i id="spinner" class="fas fa-sync fa-spin"></i></span>`).appendTo(
      $(`#${ids.root}`)
    );
  else $(`#${ids.root} #${ids.spinner}`).show();
  if ($(`#${ids.clone}`).length == 0)
    $("<div/>")
      .attr("id", ids.clone)
      .appendTo($(`#${ids.root}`));
  ...
  Promise.all(promises).then(function (args) {
    ...
    const violinPlotData = _parseGeneExpressionForViolin(
      args[1],
      tissueIdNameMap,
      groupColorDict,
      false
    ...
    if (!violinPlotData.length) {
      $(`#${ids.toolbar}`).remove();
      $(
        `<div id="gene-exp-vplot">No gene expression data found for ${geneId}</div>`
      ).appendTo(`#${ids.root}`);
    ...
    violinPlot.geneJson = { allData: args[1] };
    violinPlot.unit =
      violinPlotData.length > 0 ? ` ${violinPlotData[0].unit}` : "";
 ...
 */
function _setViolinPlotDimensions(
  width = 1200,
  height = 250,
  margin = _setViolinPlotMargins()
) {
  return {
    width: width,
    height: height,
...
function _addToolbar(vplot, tooltip, ids, urls) {
  ...
  toolbar.createDownloadSvgButton(
    ids.buttons.download,
    ids.svg,
    "gene-exp-plot.svg",
    ids.clone
  );
  ...
  $(
    `<button class="btn btn-outline-secondary" id="${ids.buttons.logScale}">Log</button>`
  ).appendTo(scaleButtonGroup);
  $(
    `<button class="btn btn-outline-secondary" id="${ids.buttons.linearScale}">Linear</button>`
  ).appendTo(scaleButtonGroup);
  ...
  $(
    `<button class="btn btn-outline-secondary fa fa-caret-up" id="${ids.buttons.ascSort}"></button>`
  ).appendTo(numSortButtonGroup);
  $(
    `<button class="btn btn-outline-secondary fa fa-caret-down" id="${ids.buttons.descSort}"></button>`
  ).appendTo(numSortButtonGroup);
  ...
  $(
    `<button class="btn btn-outline-secondary" id="${ids.buttons.outliersOn}">On</button>`
  ).appendTo(outliersButtonGroup);
  $(
    `<button class="btn btn-outline-secondary" id="${ids.buttons.outliersOff}">Off</button>`
  ).appendTo(outliersButtonGroup);

  selectAll(`#${ids.plotOptionsModal} .modal-body button`).classed(
    "active",
    false
  );
  ...
  $(`#${ids.plotOptionGroups.scale} button`).on("click", function (e) {
    ...
    btn.classed("active", true);
    vplot.gpConfig.scale =
      e.target.id == ids.buttons.logScale ? "log" : "linear";
    _redrawViolinPlot(vplot, ids);
  });
  ...
  $(`#${ids.plotOptionGroups.outliers} button`).on("click", function (e) {
    let btn = select(this);
    if (btn.classed("active")) return;
    selectAll(`#${ids.plotOptionGroups.outliers} button`).classed(
      "active",
      false
    );
    btn.classed("active", true);
  ...
  $(`#${ids.plotOptionGroups.differentiation} button`).on(
    "click",
    function (e) {
      let btn = select(this);
      if (btn.classed("active")) return;
      selectAll(`#${ids.plotOptionGroups.differentiation} button`).classed(
        "active",
        false
      );
      btn.classed("active", true);
 ...
 * @private
 */
function _parseGeneExpressionForViolin(
  data,
  idNameMap = undefined,
  colors = undefined,
  useLog = true
  ...
  data[attr].forEach((d) => {
    ...
    d.group =
      idNameMap === undefined
        ...
        : idNameMap[d.tissueSiteDetailId];
    d.label = d.subsetGroup === undefined ? d.geneId : d.subsetGroup;
    d.color =
      colors === undefined
        ? "#90c1c1"
        : d.subsetGroup == "allData"
        ? `#${colors[d.tissueSiteDetailId]}`
        : colors[d.subsetGroup];
  });
...
function _addViolinTissueColorBand(plot, dom, tissueDict, loc = "top") {
  ...
  tissueG
    .selectAll(".tcolor")
    .data(plot.scale.x.domain())
    .enter()
    .append("rect")
    .classed("tcolor", true)
    .attr("x", (g) => plot.scale.x(g))
    .attr("y", (g) =>
      loc == "top" ? plot.scale.y.range()[1] : plot.scale.y.range()[0]
    )
...
function _customizeTooltip(vplot) {
  ...
  violinGs.on("mouseover", (d, i, nodes) => {
    let med =
      vplot.gpConfig.scale == "log" ? Math.pow(10, d.median) - 1 : d.median;
    ...
    if (!vplot.gpConfig.subset) {
      vplot.tooltip.show(
        d.group +
          ...
          `n = ${d.values.length}` +
          "<br/>" +
          `Median${vplot.unit}: ${med.toPrecision(4)}` +
          "<br/>"
      );
    } else {
      vplot.tooltip.show(
...
function _customizePlot(vplot, ids) {
  ...
  if (vplot.gpConfig.title !== undefined)
    vplot.addPlotTitle(svg, vplot.gpConfig.title);
  _updateOutlierDisplay(vplot, ids);
  if (!vplot.gpConfig.subset) {
...
function _updateOutlierDisplay(vplot, ids) {
  selectAll(`#${ids.svg} path.violin`).classed(
    "outlined",
    !vplot.gpConfig.showOutliers
  );
...
function _redrawViolinPlot(vplot, ids) {
  ...
  let newData = vplot.gpConfig.subset
    ? vplot.geneJson.subsetData
    : vplot.geneJson.allData;
  const violinPlotData =
    vplot.gpConfig.scale == "log"
      ? _parseGeneExpressionForViolin(
          newData,
          vplot.tIdNameMap,
          vplot.groupColorDict
        )
      : _parseGeneExpressionForViolin(
          newData,
          vplot.tIdNameMap,
  ...
  vplot.data = violinPlotData.filter(
    (d) => filteredTissues.indexOf(d.group) != -1
  );
  let sortData = vplot.sortData.filter((d) =>
    filteredTissues.includes(d.group)
  );
