...
export function transcriptTracks(par = transcriptTracksConfig) {
  ...
  let viewer = new IsoformTrackViewer(
    par.data.transcripts,
    par.data.exons,
    undefined,
    config
  );
...
export function bubblemap(par = bubblemapDemoConfig) {
  let margin = {
    ...
    top: par.marginTop,
    right: par.marginRight,
    bottom: par.showLabels
      ? par.marginBottom + par.columnLabelHeight
      : par.marginBottom,
  };
  let inWidth =
    par.width - (par.labels.row.width + par.marginLeft + par.marginRight);
  let inHeight =
    par.height - (par.labels.column.height + par.marginTop + par.marginBottom);
  if (par.useCanvas) {
    let bmapCanvas = new BubbleMap(
      par.data,
      par.useLog,
      par.logBase,
      par.colorScheme
    ...
    bmap.drawSvg(
      svg,
      { w: inWidth, h: inHeight, top: 0, left: 0 },
      par.colorScaleDomain,
      0,
      par.labels
    );
    ...
    bmap.drawBubbleLegend(
      svg,
      { x: 500, y: -40, title: "-log10(p-value)" },
      5,
      "-log10(p-value)"
    );
  }
...
export function heatmap(par = heatmapDemoConfig) {
  ...
  h.draw(
    ...
    par.columnLabelAngle,
    false,
    par.columnLabelPosAdjust
  );
...
export function dendroHeatmap(par = dendroHeatmapDemoConfig) {
  ...
  let tooltipId = `${par.id}Tooltip`;
  let dmapConfig = new DendroHeatmapConfig(
    par.width,
    par.rowTreePanelWidth,
    par.colTreePanelHeight,
    margin
  );
  let dmap = new DendroHeatmap(
    par.data.colTree,
    par.data.rowTree,
    par.data.heatmap,
    par.colorScheme,
