...
export function render(par, geneId, urls = getGtexUrls()) {
  ...
  json(urls.geneId + geneId, { credentials: "include" }).then((data) => {
    let gene = parseGenes(data, true, geneId);
    // report the gene info
    $(`#${par.divGeneInfo}`).empty();
    $("<span/>")
      .html(
        `<span><strong><i>${gene.geneId}</i></strong> (${gene.description}), ${gene.chromosome}:${gene.start}-${gene.end} (${gene.strand})`
      )
      .appendTo($(`#${par.divGeneInfo}`));

    // Only fetch eQTLs if gene has any: -DM
    if (!gene.hasEqtl) {
      $("#messageBox").text("No eQTLs found for this gene in any tissue");
      return;
    }

    let promises = [
      json(urls.tissue, { credentials: "include" }),
      json(urls.exon + gene.geneId, { credentials: "include" }),
      json(urls.singleTissueEqtl + gene.geneId, { credentials: "include" }),
    ];
...
function setDimensions(par) {
  par.margin = {
    left:
      par.marginLeft +
      par.focusPanelLabels.row.width +
      par.focusPanelLabels.row.adjust,
    top: par.marginTop,
  ...
  par.focusPanelHeight =
    par.inHeight - (par.legendHeight + par.miniPanelHeight);
  if (par.focusPanelHeight < 0)
    throw "Config error: focus panel height is negative.";
 ...
 * @param gene {Object} containing attr: geneId
...
function renderBubbleMap(
  ...
  let miniG = bmapSvg
    .append("g") // global bubble map <g>
    .attr("class", "context")
    .attr("id", "miniG")
    .attr("transform", `translate(${par.margin.left}, ${par.margin.top})`);

  let focusG = bmapSvg
    .append("g") // zoomed bubble map <g>
    .attr("id", "focusG")
    .attr("class", "focus")
    .attr(
      "transform",
      `translate(${par.focusPanelMargin.left}, ${par.focusPanelMargin.top})`
    );
  ...
  $(`#${par.divInfo}`).text(
    "Total significant associations: " + par.data.length
  );

  ///// Below are custom features and functionality

  //-- override bubble mouseover tooltip
  focusG.selectAll(".bubble-map-cell").on("mouseover", function (event, d) {
    let selected = select(this);
    let rowClass = selected.attr("row");
    let colClass = selected.attr("col");
    focusG
      .selectAll(".bubble-map-xlabel")
      .filter(`.${rowClass}`)
      .classed("highlighted", true);
    focusG
      .selectAll(".bubble-map-ylabel")
      .filter(`.${colClass}`)
      .classed("highlighted", true);
    selected.classed("highlighted", true);
    let displayValue =
      d.displayValue === undefined
        ? parseFloat(d.value.toExponential()).toPrecision(4)
        : d.displayValue;
    let displaySize =
      d.rDisplayValue === undefined ? d.r.toPrecision(4) : d.rDisplayValue;
    let displayX = d.displayX === undefined ? d.x : d.displayX;
    let displayY = d.displayY === undefined ? d.y : d.displayY;
    bmap.tooltip.show(
      `Column: ${displayX} <br/> Row: ${displayY}<br/> NES: ${displayValue}<br/> p-value: ${displaySize}`
    );
  });
  ...
  bmap.brushEvent = (event) => {
    ...
    let focusDomain = updateFocusView(par, bmap, bmapSvg, event);
    ...
    // Always render so that it removes old markers and only draws them if necessary:
    renderGeneStartEndMarkers(bmap, bmapSvg, false);
  ...
  bmap.drawColorLegend(
    bmapSvg,
    { x: par.focusPanelMargin.left, y: par.focusPanelMargin.top - 50 },
    4,
    "NES"
  );

  // Initialize window around TSS -DM
  var variants = bmap.xScaleMini.domain();
  var tssIndex = variants.indexOf(bmap.tss); // bmap.tss is nearest variant to TSS to its left
  var brushFirst = variants[max([0, tssIndex - 39])];
  var brushLast = variants[min([tssIndex + 40, variants.length - 1])];
  var brushStart = bmap.xScaleMini(brushFirst);
  var brushEnd = bmap.xScaleMini(brushLast) + bmap.xScaleMini.bandwidth();
  ...
  miniG
    ...
    .call(bmap.brush.move, [brushStart, brushEnd]);

  // hide the mini map when the focus map includes all eQTLs
  if (bmap.xScaleMini.domain().length == bmap.xScale.domain().length)
    miniG.style("display", "none");
 ...
 * @param event {Event} the d3 brush event
 * @returns {*}
 */
function updateFocusView(par, bmap, bmapSvg, event) {
  ...
  bmapSvg
    .select("#focusG")
    .selectAll(".bubble-map-cell")
    .attr("cx", (d) => {
      let x = bmap.xScale(d.x);
      return x === undefined
        ? bmap.xScale.bandwidth() / 2
        : x + bmap.xScale.bandwidth() / 2;
    })
    .attr("r", (d) => {
      let x = bmap.xScale(d.x);
  ...
  bmapSvg
    ...
    .style("font-size", () => {
      let size =
        Math.floor(bmap.xScale.bandwidth() / 2) > 10
          ? 10
          : Math.floor(bmap.xScale.bandwidth() / 2);
      return `${size}px`;
    })
    .style("display", (d) => {
      let x = bmap.xScale(d);
...
function renderTissueBadges(tissues, bmap, bmapSvg) {
  ...
  g.append("ellipse")
    ...
    .attr(
      "cy",
      (d) => bmap.yScale(d.tissueSiteDetailId) + bmap.yScale.bandwidth() / 2
    )
  ...
  g.append("text")
    ...
    .attr(
      "y",
      (d) => bmap.yScale(d.tissueSiteDetailId) + bmap.yScale.bandwidth() / 2 + 3
    )
    .attr("fill", "#ffffff")
    .style("font-size", "8px")
...
function findVariantsNearGeneStartEnd(gene, bmap) {
  ...
  const findLeftSideNearestNeighborVariant = (site) => {
    return variants.filter((d, i) => {
      ...
      let pos = parseFloat(d.split(":")[1]); // assumption: the variant ID has the genomic location
      ...
      let next = parseFloat(variants[i + 1].split(":")[1]) || undefined;
...
function buildVariantLookupTables(bmap) {
  ...
  nest()
    ...
    .forEach((d) => {
      let v = d.values[0];
      if (v.hasOwnProperty("snpId") === undefined)
        throw "Input Error: RS ID lookup table is not built.";
      if (v.hasOwnProperty("displayX") === undefined)
        throw "Input Error: display label lookup table is not built.";
...
function renderTssDistanceTrack(gene, bmap, bmapSvg) {
  ...
  let colorScale = scaleThreshold().domain(domain).range(range);
  ...
  g.selectAll(".track")
    ...
    .on("mouseover", function (event, d) {
      ...
      let ttContent = `${d}<br/>TSS Distance: ${dist} bp</br>`;
      ttContent = bmap.variantsInExons[d]
        ? ttContent + "Exon Region"
        : ttContent;
  ...
  $("#varLocator").keyup((e) => {
    ...
    if (v.length > 3) {
      ...
      focusG.selectAll(".bubble-map-xlabel").classed("query", (d) => {
        var bool =
          regex.test(d) ||
          regex.test(bmap.rsLookUp[d]) ||
          regex.test(bmap.varLookUp[d]);
        return bool;
      });
