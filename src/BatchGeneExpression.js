 ...
 */
export function launchTopExpressed(
  tissueId,
  heatmapRootId,
  violinRootId,
  urls = getGtexUrls(),
  ...
  const $filterInfoDiv =
    $(`#filterInfo`).length == 0
      ? $("<div/>").attr("id", "filterInfo").appendTo("#messageBox")
      : $(`#filterInfo`);
  if (filterGenes)
    $filterInfoDiv.html("Mitochondrial genes are excluded.<br/>");
...
export function searchById(
  ...
  const MAX = 100;
  const $message = $("<div/><br/>")
    .attr("class", "col-xs-12 col-md-12")
    .css("color", "firebrick")
    .appendTo(`#${heatmapRootId}`);
  let message = "";
  if (glist.length > MAX) {
  ...
  Promise.all([
    json(urls.tissue, { credentials: "include" }),
    json(urls.geneId + glist.join(","), { credentials: "include" }),
  ])
    .then(function (args) {
      ...
      json(fetchUrl, { credentials: "include" })
        .then(function (eData) {
          ...
          if (dataMessage !== undefined) {
            ...
            let cellW =
              tlist === undefined
                ? Math.ceil(window.innerWidth / tissues.length)
                : Math.ceil(window.innerWidth / tlist.length);
            cellW =
              cellW > maxCellW ? maxCellW : cellW < minCellW ? minCellW : cellW; // this ensures a reasonable cellW
            let dmapMargin = { top: 50, right: 250, bottom: 170, left: 10 };
            let leftPanelW = 100;
            let rootW =
              tlist === undefined
                ? cellW * tissues.length +
                  leftPanelW +
                  dmapMargin.right +
                  dmapMargin.left
                : cellW * tlist.length +
            ...
            const config = new DendroHeatmapConfig(
              rootW,
              leftPanelW,
              100,
              dmapMargin,
            ...
            const dmap = new DendroHeatmap(
              eData.clusters.tissue,
              eData.clusters.gene,
              expression,
              "YlGnBu",
              2,
            ...
            const tissueDict = tissues.reduce((a, d) => {
              if (!d.hasOwnProperty("tissueSiteDetailId"))
                throw "tissue has not attr tissueSiteDetailId";
            ...
            select("#" + dmap.config.panels.main.id)
              .selectAll(".exp-map-xlabel")
              .text((d) =>
                tissueDict[d] === undefined ? d : tissueDict[d].tissueSiteDetail
              );

            select("#" + dmap.config.panels.main.id)
              .selectAll(".exp-map-ylabel")
  ...
  if (genes.length == 0) message = "Fatal Error: the gene list is empty.<br/>";
    ...
    if (genes.length < input.length) {
      ...
      if (missingGenes.length > 0)
        message = `Warning: Not all genes are found: ${missingGenes.join(
          ","
        )}<br/>`;
    }
  }
...
function _addTissueColors(dmap, tissueDict) {
  ...
  cells
    ...
    .merge(cells)
    .style("fill", (d) =>
      tissueDict[d] === undefined ? "#000000" : `#${tissueDict[d].colorHex}`
    );
 ...
 */
function _customizeMouseEvents(
  dmap,
  tissueDict,
  geneDict,
  urls = getGtexUrls()
  ...
  const cellMouseover = function (event, d) {
    ...
    let tissue =
      tissueDict[d.x] === undefined ? d.x : tissueDict[d.x].tissueSiteDetail;
    // let gene = geneDict[d.y]===undefined?d.y:geneDict[d.y].geneSymbol;
    let gene = d.y;

    tooltip.show(
      `Tissue: ${tissue}<br/> Gene: ${gene}<br/> Median TPM: ${parseFloat(
        d.displayValue.toExponential()
      ).toPrecision(4)}`
    );
  };
  ...
  const ylabelClick = function (event, d) {
    let s = select(this);
    let action = "";
...
function _renderViolinHelper(data, dmap, tissueDict) {
  ...
  if ($(tooltipId).length == 0)
    $("<div/>").attr("id", id.tooltip).appendTo($("body")); // create it if not already present on the demo document
  ...
  const violinLegendW = Number(
    dom.select("#violinLegend").select("rect").attr("width")
  );
...
function _addViolinTissueColorBand(plot, dom, tissueDict, loc = "top") {
  ...
  tissueG
    ...
    .attr("y", (g) =>
      loc == "top" ? plot.scale.y.range()[1] - 5 : plot.scale.y.range()[0] - 5
    )
...
function _sortTissues(xlist, dmap, tissueDict) {
  ...
  dmap.objects.heatmap.redraw(
    dom,
    xlist,
    dmap.objects.heatmap.yList,
    dimensions
  );
