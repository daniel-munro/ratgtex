...
export default class BubbleMap {
  ...
  addTooltip(parentId) {
    ...
    if ($(`#${tooltipId}`).length == 0)
      $("<div/>").attr("id", tooltipId).appendTo(parent);
  ...
  drawCanvas(
    ...
    this.data.forEach((d) => {
      ...
      context.arc(
        this.xScale(d.x) + this.xScale.bandwidth() / 2,
        this.yScale(d.y),
        this.bubbleScale(d.r),
        0,
        2 * Math.PI
      );
      context.fill();
    ...
    if (cl.show) {
      this.xScale.domain().forEach((d) => {
        ...
        context.textAlign = cl.textAlign;
        context.translate(
          this.xScale(d) + this.xScale.bandwidth() / 2 - 3,
          this.yScale.range()[1] + cl.adjust
        );
  ...
  drawCombo(
    ...
    if (addBrush) {
      let brush = brushX()
        ...
        .on("brush", (event) => {
          this._brushed(focusDom, labelConfig, event);
        });
      miniDom
        .append("g")
        .attr("class", "brush")
  ...
  drawSvg(
    ...
    bubbles
      ...
      .on("mouseover", function (event, d) {
        ...
        tooltip.show(
          `Column: ${displayX} <br/> Row: ${displayY}<br/> Value: ${displayValue}<br/> Size: ${displaySize}`
        );
      })
    ...
    if (cl.show) {
      ...
      let xLabels = dom
        ...
        .style("font-size", () => {
          let size =
            Math.floor(this.xScale.bandwidth() / 2) > 12
              ? 12
              : Math.floor(this.xScale.bandwidth() / 2);
          return `${size}px`;
        })
        .attr("transform", (d) => {
    ...
    if (rl.show) {
      ...
      let yLabels = dom
        ...
        .style("font-size", () => {
          let size =
            Math.floor(this.yScale.bandwidth() / 1.5) > 14
              ? 14
              : Math.floor(this.yScale.bandwidth() / 1.5) < 10
              ? 10
              : Math.floor(this.yScale.bandwidth() / 1.5);
          return `${size}px`;
        })
        .attr("transform", (d) => {
  ...
  drawBubbleLegend(
    dom,
    legendConfig = { x: 0, y: 0, title: "Bubble legend" },
    ticks = 5,
    unit = ""
  ) {
    ...
    let interval =
      (this.bubbleScale.domain()[1] - this.bubbleScale.domain()[0]) / ticks;
    ...
    g.append("text")
      .attr("class", "color-legend")
      .text((d) =>
        this.useLog ? Math.pow(base, d).toPrecision(2) : d.toPrecision(2)
      )
  ...
  _setMiniScales(dimensions = { w: 1000, h: 600, top: 20, left: 20 }, cDomain) {
    if (this.xScaleMini === undefined)
      this.xScaleMini = this._setXScaleMini(dimensions);
    if (this.yScaleMini === undefined)
      this.yScaleMini = this._setYScaleMini(dimensions);
    if (this.colorScale === undefined)
      this.colorScale = this._setColorScale(cDomain);
  ...
  _setXScale(dim = { w: 1000, left: 20 }, xList = undefined) {
    ...
    return scaleBand() // reference: https://github.com/d3/d3-scale#scaleBand
      ...
      .padding(0.05); // temporarily hard-coded value
  }
  ...
  _setColorScale(domain) {
    ...
    let data =
      domain === undefined
        ? this.data.map((d) => (useLog ? this._log(d.value) : d.value))
        : domain;
