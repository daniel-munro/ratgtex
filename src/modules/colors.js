...
export function drawColorLegend(
  ...
  if (diverging) {
    ...
    data = data
      .concat(range.filter((d) => d != 0).map((d) => 0 - d * interval))
      .sort((a, b) => {
  ...
  if (orientation == "h") {
    ...
    g.append("text")
      .attr("class", "color-legend")
      .text((d) =>
        useLog ? (Math.pow(base, d) - 1).toPrecision(2) : d.toPrecision(2)
      ) // assuming that raw value had been adjusted by +1 to deal with log transforming zeros
      .attr("x", (d, i) => cell.w * i)
      .attr("y", 0);
  } else {
    // legend title
    dom
      ...
      .attr(
        "transform",
        `translate(${config.x}, ${config.y + cell.h * data.length})rotate(90)`
      );
