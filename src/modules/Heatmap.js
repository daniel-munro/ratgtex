...
export default class Heatmap {
  ...
  constructor(
    ...
    if ($(`#${tooltipId}`).length == 0)
      $("<div/>").attr("id", tooltipId).appendTo($("body"));
  ...
  draw(
    dom,
    dimensions = { w: 1000, h: 600 },
    angle = 30,
    useNullColor = false,
    columnLabelPosAdjust = null
  ) {
    ...
    const Y =
      columnLabelPosAdjust == null
        ? this.yScale.range()[1] + this.yScale.bandwidth() * 2
        : this.yScale.range()[1] + columnLabelPosAdjust;
    const adjust = 5;
    ...
    cells
      ...
      // .duration(2000)
      .style("fill", (d) => {
        return useNullColor && d.value == 0
          ? nullColor
          : this.useLog
          ? this.colorScale(this._log(d.value))
  ...
  cellMouseover(d, dom, selected) {
    ...
    const displayValue =
      d.displayValue === undefined
        ...
        : d.displayValue;
    this.tooltip.show(
      `Column: ${d.x} <br/> Row: ${d.y}<br/> Value: ${displayValue}`
    );
  }
  ...
  _setXScale(width, newList = undefined) {
    ...
    this.xScale = scaleBand()
      .domain(this.xList)
      .range([0, width])
      .padding(0.05); // TODO: eliminate hard-coded value
  }
