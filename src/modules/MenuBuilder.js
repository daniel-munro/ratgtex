...
import { getGtexUrls, parseTissues } from "./DataParser";
 ...
 * @param groups {Dictionary} of lists of tissues indexed by the group name, this is created by DataParser:parseTissueSites()
...
export function createTissueGroupMenu(
  ...
  const $sections = range(0, sections).map((d) => {
    return $(
      `<div id="section${d}" class="col-xs-12 col-md-${colSize}">`
    ).appendTo($(`#${domId}`));
  });
      ...
      if ($(this).is(":checked")) {
        ...
        if ($(this).hasClass("tissueGroup")) {
          ...
          if (groups[groupName].length == 1) {
            useNames
              ? queryTissues.push(groups[groupName][0].name)
              : queryTissues.push(groups[groupName][0].id);
          }
        } else {
          // this input item is a tissue site
          useNames
            ? queryTissues.push($($(this).siblings()[0]).text())
            : queryTissues.push(id);
        }
      }
    });
