...
export default class Dendrogram {
  ...
  _drawHTree(dom) {
    const setY = (node) => {
      if (node.children === undefined) {
        ...
        node.y =
          node.children.reduce((sum, d) => sum + d.y, 0) / node.children.length;
      }
    };
    const setX = (node) => {
    ...
    dom
      ...
      .attr("x1", (d) => d.x)
      .attr("x2", (d) =>
        d.data.length ? d.x - this.xScale(d.data.length) : d.x
      )
    ...
    dom
      ...
      .call(
        axisBottom(this.xScale).ticks(3)
  ...
  _sortNodesByLevel() {
    ...
    return this.root
      .descendants()
      .sort(
        (a, b) => a.height - b.height || ascending(a.data.length, b.data.length)
      );
  }
  ...
  _drawVTree(dom) {
    const setX = (node) => {
      if (node.children === undefined) {
        ...
        // the y coordinate of an internal node is the average y from its children
        node.x =
          node.children.reduce((sum, d) => sum + d.x, 0) / node.children.length;
      }
    };
    const setY = (node) => {
    ...
    dom
      ...
      .attr("y1", (d) => d.y)
      .attr("y2", (d) =>
        d.data.length ? d.y - this.yScale(d.data.length) : d.y
      )
