var EqtlDashboard = (function (exports) {
  ...
  ((function sourceRandomLogNormal(source) {
    var N = randomNormal.source(source);
  ...
  var gamma = (function sourceRandomGamma(source) {
    var randomNormal$1 = randomNormal.source(source)();
  ...
  class GroupedViolin {
      ...
      _drawViolin(dom, entry, showWhisker, gIndex, showOutliers, numPoints){
          ...
          if (entry.values.length >= numPoints && vertices.length && this._validVertices(vertices)) {
              ...
              let zMax = max$1(vertices, (d)=>Math.abs(d[1])); // find the abs(value) in entry.values
              this.scale.z
                  .domain([-zMax, zMax])
                  .range([this.scale.subx(entry.label), this.scale.subx(entry.label) + this.scale.subx.bandwidth()]);
              ...
              if(showWhisker){
                  ...
                  const upper = max$1(entry.values.filter((d)=>d<=q3+(iqr*1.5)));
                  const lower = min$1(entry.values.filter((d)=>d>=q1-(iqr*1.5)));
                  dom.append("line")
